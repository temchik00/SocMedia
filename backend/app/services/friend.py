...
class FriendService:
    ...
    def add_friend(self, friend_id, user_id) -> User:
        friend = self.session.query(tables.User).\
            filter_by(id=friend_id).first()
        if not friend:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
        friendship = self.session.query(tables.Friend).\
            filter_by(user_id=user_id, friend_id=friend_id).first()
        if friendship:
    ...
    def get_all_friends(self, user_id) -> List[User]:
        friendships = self.session.query(tables.Friend).\
    ...
    def delete_friend(self, friend_id, user_id):
        ...
        if not friendship:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
        friendship1 = self.session.query(tables.Friend).\
            filter_by(user_id=user_id, friend_id=friend_id).first()
        self.session.delete(friendship1)
        friendship2 = self.session.query(tables.Friend).\
            filter_by(user_id=friend_id, friend_id=user_id).first()
