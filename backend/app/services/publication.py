...
class PublicationService:
    ...
    def get_publication(self, publication_id: int) -> Publication:
        publication = self.session.query(tables.Publication).\
            filter_by(id=publication_id).first()
        if not publication:
    ...
    def create_publication(self, publication_info: PublicationCreate,
                           user_id: int) -> Publication:
    ...
    def get_likes(self, publication_id: int) -> PublicationLikes:
        ...
        likes = self.session.query(tables.Like).\
            filter(tables.Like.publication_id == publication_id).all()
        user_ids = []
        for like in likes:
    ...
    def toggle_like(self, publication_id: int, user_id: int):
        self.get_publication(publication_id)
        like = self.session.query(tables.Like).\
            filter_by(publication_id=publication_id, user_id=user_id).first()
        if like:
